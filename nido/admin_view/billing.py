...
def new_recurring_charge():
    charged_to = request.form["charged_to"].split(":")
    ...
    try:
        frequency_skip = int(request.form[f"{new_charge.frequency.name}_skip"])
    except:
        pass
    if charged_to[0] == "user":
        new_charge.user_id = charged_to[1]
        new_charge.u_community_id = get_community_id()
    elif charged_to[0] == "residence":
        new_charge.residence_id = charged_to[1]
